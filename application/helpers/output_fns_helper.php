...
?>
  <body>
	<div class = "container-fluid" id = "outmost" style = "background:url(images/background.jpg);">
		<div class = "container" style = "background-color: white;border: 2px solid black;">
  
		 
		  <img src = "<?php echo base_url()."images\pyramid.gif"?>" 
						 width = 73 height = 49 alt ="" valign = "middle" />
		  <div class="jumbotron" style="color:#FFFFFF;background-color:#0066FF;"><h2><?php echo $title;?></h2></div>
<?php
}
...
function display_account_form($user='', $realname='', $mimetype='') 
{                              
  if($realname)
    $title = $realname;
  else
...
?>
  <center>
  <form method='post' action='<?php echo base_url()."index.php?action=store-account"?>'>
  <table bgcolor='#cccccc' cellpadding = 6 cellspacing = 0 border =0>
   ...
   <tr>
     <td>Email Address:</td>
     <td><input type = "text" name = "user" maxlength = 100 
          value = '<?php echo $user?>'></td>
   </tr>
   <tr>
     <td>Requested user Format:</td>
     <?php
...
{
  // display form asking for user and password
?>
  <center>
  <form method="post" action="<?php echo base_url()."index.php?action="?><?php echo $action?>" role="form" class="form-horizontal">
  <h4>Please Log In</h4>
...
function display_form_button($button)
{
  ...
  echo "<div class='form-group'><center><input class='btn btn-default' type = 'image' src = '".base_url()."images/".$button.".gif' 
                 border = 0 ";
  echo 'width = 149 height = 43 ';
  echo "alt = '".format_action($button)."' /></a></center></div>";
}

function display_button($button, $extra_parameters = '')
{
  //display one of our standard buttons as a href
  $url = base_url()."index.php?action=$button";
  if($extra_parameters)
    $url .= $extra_parameters;

  echo "<button type='button' class='btn btn-primary' style='background-color:#E68A00'><a href='$url' class='btn btn-primary'>";
  echo format_action($button);
  echo "</a></button>";
}
...
function display_mail_form($user, $listid=0)
{
...
?>
   <br />
   <center>
   <form action=<?php echo base_url()."index.php?action=store-list"?> method="post">
   <table cellpadding=2 cellspacing=0 bgcolor="#cccccc">
